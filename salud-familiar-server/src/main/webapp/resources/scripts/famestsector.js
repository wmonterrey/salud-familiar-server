var SearchSector = function () {
    ...
    return {
        ...
        init: function (parametros) {
            ...
            var table1  = $('#lista_sectores').DataTable({
            ...
            jQuery('#lista_sectores_wrapper .dataTables_filter input').addClass("form-control input-medium"); // modify table search input
            jQuery('#lista_sectores_wrapper .dataTables_length select').addClass("form-control input-small"); // modify table per page dropdown
            jQuery('#lista_sectores_wrapper .dataTables_length select').select2({
            ...
            $('#municipio').change(
            		function() {
            			$('#lista_sectores_new').removeAttr('disabled');
            			form.submit();
            		}
            	);	
            ...
            function getSectores() {
    			App.blockUI(pageContent, false);
    			$.getJSON(parametros.upSecUrl, {
    				codMunicipio : $('#municipio').val(),
    				ajax : 'true'
    			}, function(data) {
    				var len = data.length;
    				for ( var i = 0; i < len; i++) {
						var editUrl = parametros.editUrl + '/'+data[i].famEstSectorId;
						var delUrl = parametros.delUrl + '/'+data[i].famEstSectorId;
						table1.fnAddData(
    							[data[i].sector.nombre, data[i].famEstimadas,
