        ...
        init: function (parametros) {
            ...
            function getVisitas() {
    			...
    			$.getJSON(parametros.reportUrl, {
    				...
    				title = parametros.visits + ' - ' + $('#comunidad option:selected').text();
    				var d = new Date();
                    fecha=d.toLocaleString(parametros.language);
    				var table1 = $('#resultados').dataTable( {  
    	                "aoColumns" : [null,null,{sClass: "aw-right" },{sClass: "aw-right" },null,null,null],
    				...
    				for ( var i = 0; i < len; i++) {
						...
						table1.fnAddData(
    							[data[i].familia.comunidad.nombre, d.yyyymmdd(), data[i].familia.numVivienda, data[i].familia.numFamilia, data[i].tipoVisita, 
    							 data[i].personaVisita, data[i].personaVisitaProfesion.valor]);
    				}
    				App.unblockUI(pageContent);
    			})
    			.fail(function() {
